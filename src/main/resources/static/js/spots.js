    ...
    if (selectedCategory === '15') {
        fetch(`/api/spots/detail?contentId=${spot.contentid}&contentTypeId=15&lang=${currentLang}`)
            ...
            .then(data => {
                ...
                if (startRaw || endRaw) {
                    html += `<p>📅 <strong>기간</strong>: ${fmt(startRaw)} ~ ${fmt(endRaw)}</p>`;
                }
                if (place) {
                    html += `<p>📍 <strong>행사 장소</strong>: ${place}</p>`;
                }
                if (homepage) {
                    const url = homepage.replace(/<[^>]+>/g, '').trim();
                    if (url) html += `<p>🔗 <strong>홈페이지</strong>: <a href="${url}" target="_blank" style="color:#4a90e2;">${url}</a></p>`;
                }
                html += '</div>';
